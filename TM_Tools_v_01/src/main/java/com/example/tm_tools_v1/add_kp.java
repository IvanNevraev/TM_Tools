...
public class add_kp extends AppCompatActivity {
    ...
    public void click_add_kp (View v){
        ...
        //Проверяем обязательные для ввода поля
        if(add_kp_number.getText().toString().length()==0||add_kp_name.getText().toString().length()==0){
            Toast.makeText(this,"Номер и название КП обязательные поля", Toast.LENGTH_LONG).show();
            return;
        }
        cv.put("number_kp",add_kp_number.getText().toString());
        cv.put("name_kp",add_kp_name.getText().toString());
        cv.put("key_type",add_kp_key_type_spiner.getSelectedItem().toString());
        ...
        cv.put("type_relay",add_kp_type_relay_spiner.getSelectedItem().toString());
        cv.put("type_restart_device",add_kp_type_restart_device_spiner.getSelectedItem().toString());
